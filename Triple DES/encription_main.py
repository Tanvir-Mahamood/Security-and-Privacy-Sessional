...
def F(R_32: str, subKey48: str):
    ...
    xor_result = bin(int(R_expanded, 2) ^ int(subKey48, 2))[2:].zfill(48) # 48 bits
...
def DES_Algorithm(message: str, subKeys48: list, opType: str):
    ...
    for iter in range(start, stop, step): # 16 rounds
        F_output = F(R_32, subKeys48[iter]) # 32 bits

        # Final XOR and swap
        R_new = bin(int(L_32, 2) ^ int(F_output, 2))[2:].zfill(32) # 32 bits
        L_new = R_32
